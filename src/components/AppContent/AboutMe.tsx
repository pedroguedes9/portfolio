...
export const AboutMe = ({currentLanguage, layoutMode}:AboutMeProps) => {
    ...
    return (
        <div className={`grid grid-cols-[1fr_1.5fr] h-full select-text transition-all duration-500 ease-in-out mx-auto items-start
            ${layoutMode === "desktop-maximized" ? "max-w-6xl gap-8 p-10 pt-[17vh]" : "max-w-full gap-9 p-5 pt-4 pb-0"}`}>
            <section className={`flex flex-col items-center transition-all duration-500 ease-in-out 
                ${layoutMode === "desktop-maximized" ? "pt-0" : "pt-8"}`}>
                <img 
                ...
                className={`rounded-full object-cover border-2 border-white/20 shadow-lg shadow-violet-500/20 ring-4 ring-white/5 transition-all duration-500 ease-in-out
                    ${layoutMode === "desktop-maximized" ? "h-60 w-60" : "h-40 w-40"}`} 
                />
                <h2 className={`mt-4 font-semibold text-white/90 ${layoutMode === "desktop-maximized" ? 'text-2xl' : 'text-xl' } transition-all duration-500`}>Pedro Chaves Guedes</h2>
                <p className={` text-violet-200/80 ${layoutMode === "desktop-maximized" ? 'text-base' : 'text-sm'} transition-all duration-500`}>{translations[currentLanguage].appWindow.about.role}</p>
                ...
                className={`mt-4 font-medium text-white/90 
                    ...
                    ${layoutMode === "desktop-maximized" ? 'py-2 px-10 text-[17px]' : 'py-2 px-8 text-[15px]'}
                    `}>
            ...
            <section className={`flex flex-col gap-3 max-w-125 transition-all duration-500 ease-in-out
                ${layoutMode === "desktop-maximized" ? 'pt-0' : 'pt-4'}`}>
                <p className={`font-semibold text-white/90 ${layoutMode === "desktop-maximized" ? 'text-3xl' : 'text-xl'} transition-all duration-500`}>
                    {currentLanguage === "pt" ? "Olá!" : "Hello!"}
                </p>
                <p className={`max-w-125 leading-relaxed text-white/80 ${layoutMode === "desktop-maximized" ? 'text-[17px]' : 'text-[15px]'} transition-all duration-500`}>
                    {firstParagraph.map(part => (
                        part.highlight ? <span className="font-medium text-violet-400">{part.text}</span> : part.text
                    ))}
                </p>
                <p className={`max-w-125 leading-relaxed text-white/75 ${layoutMode === "desktop-maximized" ? 'text-[17px]' : 'text-[15px]'} transition-all duration-500`}>
